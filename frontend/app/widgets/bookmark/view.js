    ...
    columns: [
      {
        ...
        formatter: function (value, row, index) {
          return `<div class="bookmarkitem" title="${row.name}">
                      <img src="${row.icon.image}" style="width:100%;" />
                      <div class="title">
                          <p>${row.name}</p>
  ...
  if (window.parent.hasServer) {
    ...
    parent.sendAjax({
      ...
      error: function (XMLHttpRequest, textStatus, errorThrown) {
        alert("Service error: " + XMLHttpRequest.statusText + "，code " + XMLHttpRequest.status);
      },
    });
  } else {
...
function saveBookmark() {
  ...
  for (var index = arrBookmark.length - 1; index >= 0; index--) {
    if (arrBookmark[index].name == name) {
      toastr.info("The name already exists, please change！");
      return;
    }
  }
