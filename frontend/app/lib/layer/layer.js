...
!(function (window, undefined) {
  ...
  ready.record = function (layero) {
    var area = [layero.width()+1, layero.height()+1, layero.position().top, layero.position().left + parseFloat(layero.css("margin-left"))];
    ...
    var WRAP = "layui-layer-wrap",
      remove = function () {
        ...
        if (type === ready.type[1] && layero.attr("conType") === "object") {
          ...
          if (admuiIframe) {
            $(admuiIframe)
              .find("#admui-" + doms[0] + index)
              .before(wrap.css("display", wrap.data("display")).removeClass(WRAP));
          } else {
